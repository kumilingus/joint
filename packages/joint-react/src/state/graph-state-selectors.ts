...
export function createDefaultLinkMapper<Link extends GraphLink>(
  ...
  return () => {
    // Extract core link properties only
    // Styling properties (color, width, sourceMarker, targetMarker) stay in userData
    const {
      id,
      source: linkSource,
      target: linkTarget,
    ...
    // Build attrs from styling properties
    ...
    // Only set markers if they are not null (avoid rendering "undefined" in SVG)
    if (resolvedSourceMarker !== null) {
      lineAttributes.sourceMarker = resolvedSourceMarker;
    }
    if (resolvedTargetMarker !== null) {
      lineAttributes.targetMarker = resolvedTargetMarker;
    }
