...
var manhattanConfig = {
    ...
    fallbackRouter: function(vertices, opt, linkView) {
        ...
        if (!util.isFunction(routers.orthogonal)) {
            throw new Error('Manhattan requires the orthogonal router as default fallback.');
        }

        return routers.orthogonal(vertices, util.assign({}, config, opt), linkView);
    },
    ...
    obstacleMap: function(linkView, opt) {
        return (new ObstacleMap(opt)).build(linkView.paper.model, linkView.model);
    }
};
...
ObstacleMap.prototype.build = function(graph, link) {
    ...
    if (link) {
        ...
        excludedEnds = util.toArray(opt.excludeEnds).reduce(function(res, item) {
...
function router(vertices, opt, linkView) {
    ...
    // pathfinding
    var map = opt.obstacleMap(linkView, opt);
...
// public function
const obstacleMap = new ObstacleMap(manhattanConfig);
...
var paper = new joint.dia.Paper({
    ...
    beforeRender: function() {
        // should reduce number of obstacle map builds in comparison
        // with graph listen approach (true for paper async mode only)
        obstacleMap.map = {};
        obstacleMap.build(graph);
    },
    defaultRouter: (vertices, _, linkView) => {
        return router(vertices, {
            ...manhattanConfig,
            obstacleMap: (linkView) => {
                return obstacleMap;
            }
        }, linkView);
    }
